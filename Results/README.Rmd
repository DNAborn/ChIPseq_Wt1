...
#### Hists & Tables
```{r hits_tables, echo=FALSE, fig.width=12, out.width="100%"}
...
g2 <- ggplot(df,aes(x=score, y=signalValue, color=hits)) +
...
ht_list[[run]] <-  (hist_hits[[run]] + g) / (g2 + gridExtra::tableGrob(table, theme = t1) + plot_layout(widths = c(1,1)))
 (hist_hits[[run]] + g) / (g2 + gridExtra::tableGrob(table, theme = t1) + plot_layout(widths = c(1,1)))
...
```{r annotae_all_peaks, fig.width=12, fig}
