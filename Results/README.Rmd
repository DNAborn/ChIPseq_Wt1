...
#### Hists & Tables
```{r hits_tables, out.width="100%", echo=FALSE, out.height="1000px"}
...
for (run in names(hist_hits)){
  ...
  print(hist_hits[[run]] / gridExtra::tableGrob(table, theme = t1) +  plot_layout(heights = c(1,2)) )
}
...
g2 <- ggplot(all_npeaksAnno_table,aes(x=qValue, group=annotation_short, fill=annotation_short)) +
      stat_bin(alpha=0.5, position = 'identity', binwidth=1) + 
  # stat_bin(position = 'identity', binwidth=100, geom="text", aes(label=after_stat(count)), vjust=-0.5, colour="blue") +
  scale_fill_viridis_d() +
...
kable(top_hits[c(0:50),c("symbol","distanceToTSS","hits","score","signalValue","qValue")])
