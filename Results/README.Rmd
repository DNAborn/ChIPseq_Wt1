...
stats_t <- foreach(
  ...
  stats <- summarize_all(op_df, mean) %>% round(digits = 2)
  stats$id <- i
...
df <- as.data.frame(npeak_combined_all[[i]])
df$hits <- as.factor(df$hits)
score_lim <- quantile(df$score, .90)
peak_lim <- quantile(df$peak, .90)
ggplot(df,aes(x=score, y=peak, color=hits)) +
