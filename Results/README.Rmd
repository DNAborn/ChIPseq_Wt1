...
width_lim <- quantile(all_npeaksAnno_table$max.width, .99)
...
g2b <- ggplot(df,aes(x=max.width, y=signalValue, color=annotation_short)) +
  geom_point(size=1.5, position ='jitter', alpha=0.4) +
  scale_color_viridis_d(option ="turbo") +
  coord_cartesian(xlim=c(180,width_lim), ylim = c(signalValue_min,signalValue_lim)) +
  ggtitle(paste("D: signalValue per peak width (region)"))

g2c <- ggplot(df,aes(x=max.width, y=signalValue, color=distanceToTSS)) +
  geom_point(size=1.5, position ='jitter', alpha=0.4) +
...
head(top_hits_genes)
head(venn_top_list[["v2"]])
head(venn_peak_list[["v2"]])
peaks <- venn_top_list[["v2"]]
peaks$peakNames

filtered3 <- findOverlaps(top_hits_genes,peaks, type = 'any')
queryHits(filtered2) %>% duplicated() %>% summary

filtered <- npeak[-(queryHits(filtered2)),]

l <- length(rownames(peaks))
i <- 1
for (i in 1:l){
